...
struct ConfigData FLATBUFFERS_FINAL_CLASS : private flatbuffers::Table {
  ...
  bool resetMeshAsset() const {
    return GetField<uint8_t>(VT_RESETMESHASSET, 0) != 0;
  }
  bool Verify(flatbuffers::Verifier &verifier) const {
    return VerifyTableStart(verifier) &&
           ...
           VerifyField<uint8_t>(verifier, VT_RESETMESHASSET) &&
           verifier.EndTable();
  }
};
...
struct ConfigDataBuilder {
  ...
  void add_resetMeshAsset(bool resetMeshAsset) {
    fbb_.AddElement<uint8_t>(ConfigData::VT_RESETMESHASSET, static_cast<uint8_t>(resetMeshAsset), 0);
  }
...
inline flatbuffers::Offset<ConfigData> CreateConfigData(
    ...
    bool resetMeshAsset = false) {
  ConfigDataBuilder builder_(_fbb);
  builder_.add_resetMeshAsset(resetMeshAsset);
